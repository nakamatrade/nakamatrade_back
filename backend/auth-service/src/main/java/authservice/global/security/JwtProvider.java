...
public class JwtProvider {
    ...
    public TokenDto generateToken(Authentication authentication) {
        ...
        String accessToken = buildAccessToken(authentication.getName(), authorities, now);
        String refreshToken = buildRefreshToken(authentication.getName(), now);
        return new TokenDto(BEARER_TYPE, accessToken, refreshToken);
    }

    public Authentication getAuthentication(String accessToken) {
        Claims claims = parseClaims(accessToken);
        validateAuthorityClaims(claims);
        Collection<? extends GrantedAuthority> authorities = extractGrantedAuthorities(claims);
    ...
    private String buildAccessToken(String subject, String authorities, long now) {
        return Jwts.builder()
                .subject(subject)
    ...
    private void validateAuthorityClaims(Claims claims) {
        if (claims.get(AUTHORITIES_KEY) == null) {
            throw new CustomAuthException(ErrorCode.INVALID_TOKEN_AUTHORITY);
        }
    }

    private Collection<? extends GrantedAuthority> extractGrantedAuthorities(Claims claims) {
